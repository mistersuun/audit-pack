"""Run the /rj/v2 pipeline programmatically on a copy of the unfilled backup,
then diff Jour row 25 vs the user's final fixed file to find the $614 gap."""
import sys, io, time, shutil, tempfile
...
# Clone backup to temp
tmp = tempfile.NamedTemporaryFile(suffix='.xls', delete=False)
tmp.close()
shutil.copy2(BAK, tmp.name)
PIPELINE_OUT = tmp.name
print(f'Working on copy: {PIPELINE_OUT}\n')

# Parse docs (same as webapp)
...
# Build jour values via JourMapper (same as /api/rj/fill-all does)
mapper = JourMapper(
...
# Write via COM (same as fill-all)
with RJFillerCOM(PIPELINE_OUT) as f:
    f.write_geac(geac)
    ...
    # Autofill-recap-from-docs equivalent
    with open(f'{DOC}\\HOUSE_TOTALS.txt', 'rb') as fh:
        ht = HouseTotalsParser(fh.read(), filename='HOUSE_TOTALS.txt')
        ht.parse()
    with open(f'{DOC}\\90_2.pdf', 'rb') as fh:
        db = DebourseParser(fh.read(), filename='90_2.pdf')
        db.parse()
    ...
    surplus = abs(-228.44)
    argent = 11916.20
    b9 = argent - comp - dt - dt - surplus  # back-compute
    ...
    print(f'=== Pipeline produced: DC = ${dc_final:,.2f} ===\n')
...
pipeline = read_row(PIPELINE_OUT)
...
for c in sorted(set(pipeline.keys()) | set(user.keys())):
    ...
    if c in (1, 2, 3) and c != 1:  # skip formula row refs
        continue
    print(f'{c:<4} {pvn:>14,.2f} {uvn:>14,.2f} {diff:>+12,.2f}  {pf or uf}{mark}')

import os
os.unlink(PIPELINE_OUT)
print(f'\n(Temp file deleted)')
